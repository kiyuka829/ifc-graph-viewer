...
const document = data => new File([JSON.stringify({header: {ifcxVersion: 'ifcx-alpha'}, data})], 'test.ifcx');
test('composition preserves order, shallow overrides, headers, references and safe IDs', async () => {
  const source = new IfcxSource();
  const model = await source.load([await fixture('base.ifcx'), await fixture('overlay.ifcx')]);
  assert.equal(model.root.id, 'project');
  assert.equal(model.headers.length, 2);
  assert.equal(model.root.attributes.find(a => a.name === 'label').content.value, 'Merged');
  assert.deepEqual(model.root.attributes.filter(a => a.name.startsWith('nested')).map(a => a.name), ['nested::replacement']);
  assert.deepEqual(model.searchData.__proto__.items, [{id: '__proto__', displayName:'__proto__'}]);
...
test('JSON primitives and arrays survive; failed loads preserve previous model', async () => {
  ...
  assert.deepEqual(loaded.root.attributes.find(a => a.name === 'nested::array').content.value, [1,{x:2}]);
  assert.equal(loaded.root.attributes.find(a => a.name === 'nested::empty').content.value, null);
  assert.equal(loaded.root.attributes.find(a => a.name === 'nested::flag').content.value, true);
  for (const file of [new File(['{'], 'bad.ifcx'), new File(['{}'], 'bad.ifcx'), document([]), document([{path:'x',children:{self:'x'}}]), document([{path:'x',children:[]}])]) await assert.rejects(source.load([file]));
...
test('same filename layers are composed in selection order', async () => {
  const source = new IfcxSource();
  const model = await source.load([document([{path:'root',attributes:{x:1}}]),document([{path:'root',attributes:{x:2}}])]);
