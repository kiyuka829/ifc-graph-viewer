...
test('Pages accepts IFCX case-insensitively and rejects IFC, including mixed drops', () => {
  assert.equal(validateFiles(files('a.IFCX','b.ifcx'), false), 'ifcx');
  for (const selection of [files('a.ifc'), files('a.IFC','b.ifcx'), files('b.ifcx','a.ifc')]) assert.throws(() => validateFiles(selection, false), /local version/);
  assert.throws(() => validateFiles(files('a.json'), false), /Only IFCX/);
});
test('local builds accept both formats individually and reject mixed and empty selections', () => {
