  ...
  if (
    ...
    throw new Error(
      enableIfc
        ...
        : "Only IFCX files are supported.",
    );
  if (new Set(extensions).size !== 1)
    throw new Error("Load IFC and IFCX files separately.");
  if (extensions[0] === "ifc" && files.length !== 1)
    throw new Error("Load one IFC file at a time.");
